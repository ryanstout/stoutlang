...
module StoutLang
  class Metadata
    def self.add(llvm_module, metadata_name, metadata_str)
      ctx = LLVM::C.get_module_context(mod)
      ...
      LLVM::C.add_named_metadata_operand(mod, metadata_name, md_node)
    end

    def self.read(llvm_module, metadata_name)
      # Return an array of all metadata strings for the metadata_name

      num_operands = LLVM::C.get_named_metadata_num_operands(mod, metadata_name)

      operands = FFI::MemoryPointer.new(:pointer, num_operands)
      LLVM::C.get_named_metadata_operands(mod, metadata_name, operands)

      metadata_strings = []
      num_operands.times do |i|
        operand_ptr = operands.read_pointer # Read the pointer from the memory block
        operand = LLVM::C.get_operand(operand_ptr, i)
